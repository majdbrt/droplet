...
function MessageBoard() {
    ...
    return (
        <div id="mBoard" className={` overflow-auto h-screen ${!selectedChat ? 'hidden md:block' : ''} w-full flex flex-col `}>
            <div className={`${selectedChat ? 'hidden md:flex' : ''} flex sticky top-0 z-50 bg-neutral-900 border-cyan-400 w-full border-b-2 py-5 px-5 md:px-11 md:py-5 place-content-between justify-end`}>
            ...
            <div className={` flex flex-col   flex-1`}>
                <div className={`sticky top-0 md:top-16  z-10 bg-cyan-400 ${!selectedChat ? 'invisible' : ''} `}>
                ...
                <div className={` flex flex-col  ${darkTheme ? 'bg-neutral-900' : 'bg-white'}   ${!selectedChat ? 'invisible' : ''} `}>
            ...
            <form onSubmit={handleSubmit} className={`sticky bottom-0 m-0 z-auto w-full ${!selectedChat ? 'invisible' : ''} `}>
                <div id="" className={`flex h-12  border-t-2 border-neutral-500 focus:border-cyan-400 hover:border-cyan-400 ${darkTheme ? 'bg-neutral-900 text-white focus:bg-neutral-700 hover:bg-neutral-700 ' : 'bg-white text-black hover:bg-neutral-50 focus:bg-neutral-50'}`}>
                    <input type="text" className={`flex-1 border-0 px-3 bg-transparent outline-none focus:border-0 appearance-none placeholder:text-neutral-500 `} placeholder="Send a message..." onChange={handleChange} value={sendMessageInput} />
                    <button type="submit">
